...
- 操作者: AI (Codex)
- parent_trace_id: 20260812-member-loyalty-storedvalue
- 来源: 对 058567e 工作包生产级重构的再复核；结合项目级上线边界（2027-05-31 前不开放真实用户、2027-06 非自动上线）作出双阶段裁决
- 裁决: 058567e 作为 2027-05-31（含）前开发/调试/测试阶段规划基线有条件通过；不代表真实支付验收、真实用户开放或本地权威切换已通过
- 高风险阻断项（切换前必须解除）:
  - FP-1 生产导入起止双水位与窗口回放（现有有赞小程序仍可能产生真实会员 Webhook，旧快照可能覆盖新事件）
  - FP-2 local 切换模型：coupon_inventory 唯一键 (coupon_id,status,mobile) 不含 source，webhook 行占用后 local 状态无法写入；需 authority epoch / shadow audit / 状态投影解除
- 修正:
  - FP-1：eligible population 界定与四类事件验收；起止双水位、暂停后回放或版本化写入
  - FP-2：权威主体改为 Platform 本地账务域（小程序仅为用户入口），储值/积分/券/身份分别建模；拆两个独立切换窗口消除同时部署矛盾；补唯一键冲突解除设计
  - FP-3：充值 API 路径修正为 recharges.py；第一次真实支付前退款政策与持久退款聚合/异步结果/补偿恢复硬门禁
  - FP-4：拆 FP-4A 体验版准备 / FP-4B 正式发布，消除体验版与 FP-3 阶段二循环依赖
  - FP-5：类型债、flaky 根因与 mypy 渐进式门禁表述修正
  - M4 设计：券双端核销风险降级为开发阶段中风险、真实券测试与上线前硬门禁
  - 进度清单：更新提交链与 5 工作包；M3/M4 证据待归档；058567e 无独立 trace 待补
- 验证: 无代码变更，纯文档治理；VERSION 保持 0.132.1
- 版本: 0.132.1

## [2026-08-14] - docs(governance): 固化小程序正式上线前开发测试边界

- 操作者: AI (Codex)
- trace_id: 20260814-miniapp-prelaunch-boundary
- 来源: 项目负责人确认小程序用于替代现有有赞小程序；2027-06 前不开放真实用户，2027-06 也不自动上线
- 决策:
  - 截至 2027-05-31（含），小程序仅用于开发、调试和测试，不承接真实用户业务操作
  - 2027-06 只是最早候选上线窗口，必须由项目负责人明确批准后才允许开放真实用户
  - 截至 2026-08-14，尚不具备受控真实微信支付 / 退款及真实有赞券测试条件
  - 未来条件具备后，仅允许授权测试账号在事前批准下开展小额、可退款、可对账、可清理的受控真实测试；测试通过不等于正式上线
- 变更: 同步 Platform 的 AGENTS、会员账务主计划、FP-2 / FP-3 / FP-4、ADR 0007 与 ADR 索引、项目进度清单；同步 MiniApp 的 AGENTS、roadmap、手工发布验收清单与 LOGBOOK
- 验证: `git diff --check`；`python scripts/check_text_encoding.py`；`python scripts/check_project.py --skip-tests`；`python scripts/check_mistake_ledger.py`
- evidence: L2 文档决策；无运行时代码、生产配置或真实业务数据变更
- residual_risks: 正式上线检查表、批准记录及未来受控真实测试方案须在条件具备后单独闭环
- 版本: 0.132.1（纯文档治理，不触发部署）

## [2026-08-14] - docs(specs): 会员账务域后续工作包生产级重构（深度复核修正）

- 操作者: AI (Codex)
- parent_trace_id: 20260812-member-loyalty-storedvalue
- 来源: 97e7a07 四工作包深度复核；原拆分更接近索引，未达生产级执行基线
- 变更:
  - FP-1：修正脚本事实——不存在 --dry-run 参数（不传 --apply 即 dry-run）；写入目标为 member_balance/coupon_inventory/coupon_templates（不写 points_ledger）；规定 failed>0 非 0 退出、JSON 报告带 batch_id；补充限流退避/断点续跑/失败重试/Webhook 水位线/备份恢复演练
  - FP-2：积分与优惠券分别切换，逐聚合权威矩阵；优惠券切换前增加有效券基线迁移、切换水位线、写入围栏、影子读对账、观察期与反向补偿，明确 RPO/RTO；说明 authority 为进程级配置不支持单客户灰度
  - FP-3：拆为「支付/退款工程实现」与「真实商户生产验收」两阶段；补充资金状态机、交易号/退款号持久化、微信关单/退款/退款查询、跨资金补偿顺序；余额全额支付退款与券 BACK 退款接入纳入范围
  - FP-4：固定依赖图 FP-1+FP-3 体验版 → FP-4 正式发布 → FP-2 权威切换/有赞下线；新增支付关闭版本（入口隐藏/kill switch/二次开放门禁）
  - FP-5（新增）：承接仓储返回类型注解、tests/scripts 覆盖率 flaky 技术债
  - ADR 0007：正式记录 2027-06 local 权威切换决策与前置/围栏/RPO/RTO
  - 主计划：状态行与目标改为「核心主链路及 Platform 第一阶段部署完成」；M6 降低「已归档」表述，M3/M4 证据待归档
- 决策:
  - 技术债归属明确：券 BACK 退款→FP-3，真实券/非数值折扣解析→FP-1，类型注解与覆盖率 flaky→FP-5
  - 各工作包复用 parent_trace_id，执行时生成独立 trace_id
- 验证: 无代码变更，纯文档纠正；VERSION 保持 0.132.1
- 版本: 0.132.1

## [2026-08-14] - docs(specs): 会员账务域 M1–M6 评审结项（方案 B 分阶段结项 + 后续工作包拆分）

- 操作者: AI (Codex)
- trace_id: 20260812-member-loyalty-storedvalue
- 来源: docs/specs/2026-08-12-member-loyalty-storedvalue-plan.md 完整评审；核心工程完成但整份计划尚未全部完成
- 变更:
  - 主计划状态改为「核心工程完成」，新增逐里程碑状态表（✅ 已完成 / 🟡 第一阶段完成 / 🔴 外部依赖阻塞 / ⛔ 已取消）
  - M1–M6 全部标注「第一阶段完成 / 部分完成」，明确未验证边界
  - 未完成事项拆分为 4 个独立后续工作包（FP-1 会员数据正式导入 / FP-2 本地权威切换 2027-06 / FP-3 真实微信支付 / FP-4 小程序正式发布），见 docs/specs/ 对应文档
  - 项目进度与配置清单同步结项状态
- 决策:
  - local 权威切换明确保留，执行时点 2027-06，目的是以本小程序替代有赞小程序
  - 真实微信支付待商户号，不把 mock/401 探针冒充完整生产验收
- 验证: 无代码变更，纯文档结项；提交 671870d 为上一收口节点，VERSION 保持 0.132.1
- 版本: 0.132.1
...
- 版本: 0.122.1（纯文档收口提交，按 sync_version 规则不递增版本）
...
- 来源: 用户决策——「对话报号绑定」可冒报他人手机号，不做；未识别客户按新用户对待；认证到位后可选用微信官方手机号授权组件。
- 变更: 更新运行手册覆盖局限与开放问题；移除对话报号绑定候选。
- 决策明细:
  - 10,011 条已预挂 openid 客户：打开小程序静默识别，无需任何操作。
  - 未识别客户（11,175 无手机号 + 3,540 有手机号未命中）：按新用户处理，不弹强制登录。
  - 微信登录为静默 openid，无传统注册；「授权」仅指官方手机号授权组件（getPhoneNumber，微信验证真手机号，冒充不了），依赖商家认证/资质，后置不阻塞接管。
- 生产版本: 0.110.0（纯文档变更，无需部署）。
...
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **问题**: 主计划顶部、R2-A 和历史“立即下一步”仍把已完成的生产合成主体删除与 inbox 崩溃重领写成未完成，并保留旧生产版本，可能导致后续 Agent 重复执行破坏性专项。
- **修复**: 同步生产 `0.109.4`、三组 `8/8` Harness 证据、本地 D 盘长期备份事实和 Docker 后置决定；将历史启动步骤替换为当前收口顺序。
- **验证**: 主计划关键词复核、证据索引和文本编码门禁纳入提交检查。
- **边界**: 仅同步当前事实，不改变代码、生产配置或 Docker 后置决定。
...
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **发现**: 既有文本正则能覆盖常见手机号、地址、open_id 和订单号，但结构化 tool payload 中格式异常的敏感值可能因不匹配正则而保留。
- **修复**:
  - `privacy_redaction.py` 增加按键名替换手机号、地址、身份、订单和原始消息字段，并扩大非结构化区域地址识别。
  - 新增 AST 聚合门禁，自动发现模型调用模块，约束统一脱敏、单一 ChatOpenAI 工厂和 ASR SDK 窄例外；已接入 `check_project.py`。
  - 生产模式只读取离线和 LangSmith 布尔开关，不读取数据库、不调用外部模型、不输出密钥。
- **验证结果**: 聚合门禁生产专项 `8/8`；自动发现 9 个模型调用模块并全部通过；隐私定向测试 `9 passed`；生产 7 个离线/LangSmith 外发开关全部为 false。
- **结论**: R3-A 完整生产出站专项已完成；真实生产主体删除仍作为独立缺口，不以本次合成探针替代。
- **边界**: LangSmith 保持关闭，离线 QA/知识缺口/memory 保持关闭，无客户内容、订单明细、员工 ID 或密钥进入报告。

## [2026-07-12] - feat(r4-b): 配置本地 D 盘生产加密备份计划任务
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **设计**: 在生产无独立磁盘的约束下，由 Windows 本地主机主动拉取 `/dev/shm` 一致快照，在 D 盘复用 AES-256-GCM 工具加密并验证，生产根盘不承担长期备份。
- **实现**:
  - 新增 `local_production_backup.py`，负责 SSH 快照、SCP、本地加密校验、两端明文逐文件清理和每次最多一个过期备份的保留策略。
  - 新增 Windows 计划任务安装器；任务每天 03:30 运行，仅使用当前登录用户和既有 SSH key，不保存 Windows 密码。
  - 默认保留 30 天且至少 3 份；遵守文件删除红线，不循环批量删除。
- **验证结果**: 本地备份作业合同测试 `7 passed`，备份相关测试合计 `11 passed`；真实直接运行和计划任务运行均成功，计划任务 `LastTaskResult=0`，下一次运行时间为 2026-07-13 03:30。
- **证据**: D 盘现有 3 份 `.ybak`；新增两份均完成 AES-256-GCM 解密、SHA-256 和 SQLite integrity 校验；本地与生产 `/dev/shm` 均无明文临时快照残留。
- **边界**: 生产持久挂载仍未配置；Docker 继续后置；备份密钥、客户数据和数据库内容未进入仓库或报告。
...
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **备份资产**: 本地 `D:\Backups\YunxiBakeBot\bot_backup_20260712_01.ybak`；密钥位于本地受限目录 `D:\Backups\YunxiBakeBot\keys\backup.key`。不记录密钥内容或数据库摘要。
- **验证结果**:
  - 生产使用 SQLite `.backup` 生成一致性快照，大小 `20,602,880` 字节；通过 SSH 拉取到本地 D 盘。
  - 本地使用 AES-256-GCM 加密，创建阶段和独立复核均返回 `ok`；加密文件大小 `20,603,056` 字节。
  - 本地和生产侧明文临时快照均已按明确路径删除；本地密钥为 32 字节，ACL 仅当前用户。
  - 生产 systemd active，`/health` 正常，版本 `0.107.13`。
- **结论**: 已获得一份可验证的本地加密备份；它尚未挂载到生产服务器，因此不解除生产 migration job 的在线回滚门禁。
- **保留**: 按备份策略保留 30 天，过期文件由运维逐个审核清理；不使用批量删除。
- **边界**: 未执行迁移写入或回滚，未提交明文数据库、密钥或客户数据。
...
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **结果**: 生产 callback 探针 `61` 个用例中 `22` 个失败，失败全部为 `semantic rule mismatch`；失败用例的 HTTP 状态均为 200，流式回复格式、隐私模式检查均通过。
- **失败范围**: 订单/商品库存、配送知识、运营状态、待人工、客户线索和退款知识相关语义用例；未发现 HTTP 可达性、加密回复格式或隐私泄漏类别失败。
- **边界**: 报告只在仓外临时目录解析失败标志，未读取或提交回复正文；本地和生产临时文件均已清理。
- **后续**: 需要按真实业务数据与门店规则逐项校准回复语义，不能放宽统一 callback 断言或启用生产 trace sink 掩盖失败。
...
- **操作人**: AI (Codex)
- **trace_id**: 20260709-langchain-ai-layer-production-enhancement
- **背景**: P19a 已把 fixture / golden cases 的 RAG shadow 观测固化为发布证据，但真实生产检索日志尚无可接入、可脱敏、可严格验收的输入合同。当前本地 `knowledge_retrieval_logs` 没有可用真实样本，不能把“无输入”误报为真实 shadow 已准备好。
- **决策**:
  - 新增 P19b 真实 RAG shadow log 观测输入门禁，默认 readiness 通过但明确 `shadow_log_ready=false`。
  - `--require-input` 严格模式在没有脱敏输入时失败，用于未来真实日志接入后的发布门禁。
  - 输入必须声明 `contains_sensitive_data=false`，records 必须包含 `id`、`query` 和 `baseline_top_keys`。
  - 默认报告只输出 `query_hash`，只有显式 `--include-queries` 才输出已脱敏 query 文本。
  - 本切片不改变客户热路径、不改 RAG 检索模式、不写业务数据库、不调用外部 LLM。
- **改动**:
  - `scripts/report_rag_shadow_log_observability.py` - 新增真实 RAG shadow log 观测输入报告和 CLI。
  - `tests/scripts/test_report_rag_shadow_log_observability.py` - 覆盖无输入 readiness、strict 失败、脱敏输入候选复算、query 默认不输出、敏感输入拒绝和 CLI JSON。
  - `scripts/check_langchain_ai_layer_production_plan.py`、`scripts/check_project.py` - 将 P19b 纳入生产增强计划门禁和项目业务合约。
  - `docs/architecture/langchain-ai-layer-production-enhancement-plan.md`、`docs/harness-engineering/core/evidence-index.md`、`项目进度与配置清单.md`、`VERSION` - 同步 P19b 追溯记录和版本 `0.105.7`。
- **验证结果**:
  - `python -m pytest tests\scripts\test_report_rag_shadow_log_observability.py tests\scripts\test_check_langchain_ai_layer_production_plan.py -q --no-cov` 通过，11 项失败 0。
  - `python -m ruff check scripts\report_rag_shadow_log_observability.py scripts\check_langchain_ai_layer_production_plan.py scripts\check_project.py tests\scripts\test_report_rag_shadow_log_observability.py tests\scripts\test_check_langchain_ai_layer_production_plan.py` 通过。
  - `python -m ruff format --check scripts\report_rag_shadow_log_observability.py scripts\check_langchain_ai_layer_production_plan.py scripts\check_project.py tests\scripts\test_report_rag_shadow_log_observability.py tests\scripts\test_check_langchain_ai_layer_production_plan.py` 通过。
  - `python scripts\report_rag_shadow_log_observability.py --summary` 通过，输出 `shadow_log_ready=false`。
  - `python scripts\report_rag_shadow_log_observability.py --require-input --summary` 按设计失败，证明 strict gate 不会在缺少真实脱敏输入时误通过。
...
- **决策**:
  ...
  - 对零库存场景允许两种安全结果：在售商品库存 0 时必须提示暂无可售库存和替代款；商品已下架或未命中时必须提示未找到匹配商品，并说明不要把未命中当作缺货结论。
- **改动**:
  - `scripts/wecom_employee_agent_callback_semantics.py` - 新增 `required_all_term_groups`，支持同一探针的多组可接受语义结果。
  - `scripts/check_wecom_employee_agent_callback.py` - 将 probe case 的分组语义传入 callback semantic rule。
  - `scripts/wecom_employee_agent_probe_cases.py` - 移除商品库存探针对固定数字 `72` 的依赖，并补充零库存 / 未命中双安全口径。
  - `tests/scripts/test_check_wecom_employee_agent_callback.py` - 增加多组 all-terms 语义回归测试。
  - `docs/architecture/langchain-ai-layer-production-enhancement-plan.md` - 追加 P0 落地记录。
- **验证结果**:
...
## [2026-07-12] - fix(r3-r5): 不确定业务事实统一转人工
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **改动**:
  - 员工助手最终整理器在工具失败、商品/订单/知识未命中或事实无法确认时，统一返回受控转人工回复。
  - callback 探针增加明确的转人工合格分支；授权、隐私、纯文本和流式结构检查保持不变。
  - 更新两项旧测试预期，禁止未命中事实继续生成猜测性业务建议。
- **验证结果**: 员工 Agent、callback 合同和相关服务测试 `79 passed`；Ruff check/format 和 `git diff --check` 通过。
- **边界**: 尚未发布生产；生产 callback 需在发布后复跑确认 61 个用例的语义收口。
## [2026-07-12] - fix(r3-r5): callback 异常和运营判断统一转人工
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **背景**: `0.107.6` 发布后生产 callback 仍有 `22/61` 失败；复核日志确认其中 20 条为 callback 异常兜底，2 条为需要运营判断的“需要注意/要盯”问法。
- **改动**: callback 异常兜底和上述不确定运营问法统一返回受控转人工回复；callback 探针保留 HTTP、流式、隐私、授权和纯文本门禁。
- **边界**: 不放宽语义检查，不伪造业务数据；发布后重新验证 61 条探针。
## [2026-07-12] - deploy(r3-r5): callback 转人工策略生产收口
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **发布**: `ce820dc`，生产 `VERSION=0.107.8`。
- **验证结果**: 生产 systemd active；`/health=200`、`/ready=200` 且 `status=ready`；callback 探针 `61/61` 通过，失败 `0`。HTTP、流式格式、隐私和员工授权门禁均通过。
- **结论**: 无法可靠确认的商品、配送、运营、客户、退款和 callback 异常场景统一转人工；正常订单查询保持自动回复。
- **边界**: 未启用生产 trace sink；备份异盘密钥和迁移回滚仍待完成。
## [2026-07-12] - verify(r4): 生产异盘备份前置检查
- **操作人**: AI (Codex)
- **trace_id**: 20260711-global-risk-remediation
- **验证结果**: 生产仅发现根盘 `/dev/vda3`，未发现 `/mnt/backup`、`/var/backups/yunxibakebot` 或 `/opt/yunxibakebot/backups` 独立挂载；`/etc/yunxibakebot/backup.key` 不存在。
