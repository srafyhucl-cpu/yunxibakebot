...
| 状态 | Skill | 功能说明 | 问题 |
|------|-------|---------|------|
| 🟢 | `芸熙LLM守卫` | DeepSeek 调用、Function Calling、意图识别、对话循环的专项规范 | 无 |
| 🟢 | `芸熙架构守卫` | api/service/repository/models 分层边界、Webhook 幂等约束 | 无 |
| � | `芸熙文件体量守卫` | 防止单文件过大，超阻断线强制走拆分工作流 | 已修复：description 加入主动检查触发词；工作流联动升级 |
| � | `芸熙洁净代码守卫` | 零硬编码、函数设计、命名质量、可复用性 | 已修复：重定位为深度核查手册，与 AGENT.md 差异化 |
...
| 🟢 | `lark-base` | 飞书多维表格（Base）：建表、字段管理、读写记录、视图配置、工作流 |
| 🟢 | `lark-sheets` | 飞书电子表格：创建/读写单元格、追加行、查找内容、导出 |
| 🟢 | `lark-task` | 飞书任务：创建/更新任务、子任务、清单、分配成员、任务智能体 |
| 🟢 | `lark-contact` | 飞书通讯录：按姓名/邮箱查 open_id，反查员工姓名/部门/联系方式 |
| 🟢 | `lark-wiki` | 飞书知识库：管理知识空间、节点层级、文档组织 |
| � | `lark-slides` | 飞书幻灯片：创建/编辑幻灯片，读写页面内容（XML 协议） |
| 🟢 | `lark-whiteboard` | 飞书画板：导出预览图、编辑节点、支持 PlantUML/Mermaid 导入 |
| 🟢 | `lark-mail` | 飞书邮箱：收发/回复/转发邮件、管理草稿/标签/联系人/规则 |
| 🟢 | `lark-markdown` | 飞书 Markdown 文件：创建/读取/局部编辑/差异比较 |
| 🟢 | `lark-minutes` | 飞书妙记：查询妙记列表、获取 AI 总结/待办/章节、上传音视频生成妙记 |
| 🟢 | `lark-approval` | 飞书审批：查询和管理审批实例、审批任务 |
| 🟢 | `lark-attendance` | 飞书考勤：查询个人考勤打卡记录 |
| 🟢 | `lark-okr` | 飞书 OKR：管理目标/关键结果/对齐关系/进展记录 |
| 🟢 | `lark-vc` | 飞书视频会议（历史）：搜索已结束会议、获取纪要/逐字稿/参会人快照 |
| 🟡 | `lark-vc-agent` | **AI bot 入会**：让机器人代用户加入/离开**正在进行中**的会议，读取实时事件（发言/共享屏幕等）。极低频 |
| 🟡 | `lark-event` | 飞书实时事件监听：用 NDJSON 流式接收 IM 消息/群成员变更等事件，适合长连接 bot。低频 |
| 🟡 | `lark-openapi-explorer` | **飞书原生 API 探索**：当其他 lark-* skill 都无法满足需求时，从官方文档查找原始 OpenAPI 接口。定位是兜底，触发时机模糊 |
| ⚪ | `lark-shared` | 飞书 CLI 基础：首次初始化、登录认证、切换用户/bot 身份、处理权限错误 |
| ⚪ | `lark-skill-maker` | 创建 lark-cli 自定义 Skill：封装飞书 API 为可复用命令 |
| ⚪ | `lark-workflow-meeting-summary` | **工作流**：汇总指定时间范围内多次会议纪要，生成结构化报告 |
| ⚪ | `lark-workflow-standup-report` | **工作流**：组合日历日程 + 未完成任务，生成今日/本周行程摘要 |
