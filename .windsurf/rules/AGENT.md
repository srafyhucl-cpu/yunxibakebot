...
│   └── chat_test.html       # 微信高保真消息流网页端测试大看板
└── static/admin/            # 静态资源
```
...
1. **禁止单引号字符串**：统一双引号 `"`（SQL 内部字符串 and f-string 内部闭环除外）。
...
- **更新日志**：每次任务完成后更新 `LOGBOOK.md`，以及核心项目文档（进度、架构、上线检查清单等），未更新则阻断提交。
- **提交代码**：提交信息使用中文，格式：`type(scope): 中文描述`（如 `feat(chat): 新增运费关键词前置匹配`）。
- **收口顺序**：代码改动 → 测试验证 → LOGBOOK 与文档更新 → 提交 → 推送，严格按此顺序执行。
