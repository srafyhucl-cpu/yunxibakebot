...
from app.models.miniapp_address import MiniappAddress
from app.repository.base import BaseRepository


class MiniappAddressRepo(BaseRepository):
    """按小程序用户隔离读写收货地址。"""

    async def list_by_user(self, user_id: str) -> list[MiniappAddress]:
        ...
        return [MiniappAddress(**dict(row)) for row in rows]
    ...
    async def get_for_user(
        ...
        return MiniappAddress(**dict(rows[0])) if rows else None

    async def get(self, address_id: str) -> MiniappAddress | None:
        rows = await self._db.execute_fetchall(
            "SELECT id, user_id, receiver_name, receiver_phone, address, "
            "is_default, created_at, updated_at "
    ...
    async def upsert(self, item: MiniappAddress) -> None:
