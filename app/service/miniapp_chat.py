...
MINIAPP_CHAT_CHANNEL = MINIAPP_CHANNEL
...
class MiniappChatService:
    ...
    async def send_message(
        ...
        """发送一条小程序用户消息，并返回 AI 回复。"""
        normalized_content = content.strip()
        if not normalized_content:
            raise ValueError("消息内容不能为空")
        reply = await self._chat_service.handle_message(
            channel=MINIAPP_CHAT_CHANNEL,
            user_id=user_id,
            content=normalized_content,
            channel_msg_id=f"miniapp:{user_id}:{uuid4().hex}",
        )
        session = await self._session_repo.get_active(user_id, MINIAPP_CHAT_CHANNEL)
        return {
    ...
    async def request_human_transfer(
        ...
        *,
        ...
        """为小程序用户主动创建转人工工单，并返回最新客服页 payload。"""
        normalized_reason = reason.strip() or "小程序用户主动请求人工客服"
        session = await self._session_repo.get_or_create(
            SessionCreate(id="", channel=MINIAPP_CHAT_CHANNEL, user_id=user_id)
        )
    ...
    async def get_chat_status(self, *, user_id: str = MINIAPP_DEMO_USER_ID) -> dict:
        """读取当前小程序客服会话状态，供用户端展示 AI/人工接待状态。"""
        session = await self._session_repo.get_active(user_id, MINIAPP_CHAT_CHANNEL)
