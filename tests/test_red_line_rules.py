...
"""
    ...
    # ── 规则 4: api 层禁止直接导入 repository ───────────────────────────────────

    def test_rule_4_api_import_repo_violation(self) -> None:
    ...
    def test_rule_4_api_import_repo_compliant(self) -> None:
        """api 层导入 service 不误报。"""
        code = """from app.service.chat import ChatService\n"""
        self._assert_no_violations(RED_LINE_RULES[3], code)

    # ── 规则 5: service 层禁止直连 aiosqlite ────────────────────────────────────

    def test_rule_5_service_aiosqlite_violation(self) -> None:
    ...
    def test_rule_5_service_aiosqlite_compliant(self) -> None:
    ...
    # ── 规则 6: models 层禁止引用上层模块 ────────────────────────────────────────

    def test_rule_6_models_upper_violation(self) -> None:
    ...
    def test_rule_8_silent_except_violation(self) -> None:
        """检测 except: pass（单行）。"""
        code = "try:\n    risky()\nexcept: pass\n"
        self._assert_violations(RED_LINE_RULES[7], code)

    def test_rule_8_silent_except_compliant(self) -> None:
        """有日志记录的异常处理不误报。"""
        code = "try:\n    risky()\nexcept Exception:\n    logger.error('失败')\n"
...
"""
    ...
    def test_rule_10_bare_print_compliant(self) -> None:
        """使用 logger 不误报。"""
        code = """
def debug() -> None:
...
"""
    ...
    def test_rule_11_english_comment_violation(self) -> None:
        """检测英文注释。"""
        code = """
...
def get_user() -> None:
    pass
"""
        self._assert_violations(RED_LINE_RULES[10], code, expected_line_count=2)

    def test_rule_11_english_comment_compliant(self) -> None:
        """中文注释不误报。"""
        code = """
# 这是中文注释
# 根据 ID 查询用户
...
class TestRedLineRuleCoverage:
    """验证全部 11 条红线规则均已定义且各有自测。"""

    def test_all_eleven_rules_defined(self) -> None:
        """确保 RED_LINE_RULES 包含全部 11 条规则。"""
        ...
        assert len(RED_LINE_RULES) == 11, (
            f"预期 11 条规则，实际 {len(RED_LINE_RULES)} 条"
        )
    ...
    def test_each_rule_has_violation_test(self) -> None:
        ...
        violation_tests = {
            ...
            "api 层禁止直接导入 repository": "test_rule_4_api_import_repo_violation",
            "service 层禁止直连 aiosqlite": "test_rule_5_service_aiosqlite_violation",
            "models 层禁止引用上层模块": "test_rule_6_models_upper_violation",
            "禁止 SQL f-string 拼接": "test_rule_7_sql_fstring_violation",
            "禁止静默吞异常": "test_rule_8_silent_except_violation",
            "禁止硬编码密钥": "test_rule_9_hardcoded_key_violation",
            "app 内禁止裸 print": "test_rule_10_bare_print_violation",
            "禁止英文注释": "test_rule_11_english_comment_violation",
        }
    ...
    def test_each_rule_has_compliance_test(self) -> None:
        ...
        compliance_tests = {
            ...
            "api 层禁止直接导入 repository": "test_rule_4_api_import_repo_compliant",
            "service 层禁止直连 aiosqlite": "test_rule_5_service_aiosqlite_compliant",
            "models 层禁止引用上层模块": "test_rule_6_models_upper_compliant",
            "禁止 SQL f-string 拼接": "test_rule_7_sql_fstring_compliant",
            "禁止静默吞异常": "test_rule_8_silent_except_compliant",
            "禁止硬编码密钥": "test_rule_9_hardcoded_key_compliant",
            "app 内禁止裸 print": "test_rule_10_bare_print_compliant",
            "禁止英文注释": "test_rule_11_english_comment_compliant",
        }
