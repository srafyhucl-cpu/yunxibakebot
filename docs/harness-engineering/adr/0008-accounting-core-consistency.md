...
|---|---|---|---|---|---|---|
| 支付通知 `pay_notify` | 验签上下文 / `mchid` / `appid` / `trade_state=SUCCESS` / `amount.total` / `currency=CNY` / `out_trade_no` / `transaction_id` | `wechat:notify:<transaction_id>:<merchant_order_no>` | 金额 / 币种 / 商户单号与不可变快照微信腿一致 | `prepay_ready → settling → succeeded`（B3.4：明确 `prepay_ready` 到结算的合法转换；仅 `succeeded` 幂等 ACK） | 持久化成功且事件 `processed` 才 200，否则 5xx | 逐字段缺失 / 不匹配负向用例 |
| 退款通知 `refund_notify`（独立 `ProviderRefundEvent`，B3.4） | 验签上下文 / `mchid` / `appid` / 退款成功状态 / **`original_total_fen` / `refund_fen`** / `currency=CNY` / `out_refund_no` / `refund_id` | `wechat:refund_notify:<refund_id>:<out_refund_no>` | `original_total_fen` 与不可变快照微信腿 / `refund_aggregate` 原额一致；`refund_fen` 与本笔分摊一致 | **`dispatching | dispatch_unknown | accepted → confirmed_refunded`**（B3.4：覆盖受理后全部状态，通知先到 / 响应丢失均可推进；`version + lease_token` CAS） | 同上 | 原额 / 本笔额分离校验、退款号缺失、金额不一致负向用例 |
| 订单查询 `query_result`（支付） | `trade_state` / `amount.total` / `currency=CNY` / `out_trade_no` | `wechat:pay_query_result:<merchant_order_no>:<query_generation>` | 快照微信腿一致 | 按固定映射 CAS（`settling` / 关单 / 终态） | 查询回写以 `query_generation` 条件更新，陈旧拒绝 | 陈旧代次回写拒绝用例 |
...
- **独立 `ProviderRefundEvent` 合同（B3.4，评审问题 3）**：退款通知与支付通知完全独立建模，固定字段 **`original_total_fen`（原支付总额）/ `refund_fen`（本笔退款额）/ `currency=CNY` / `out_refund_no` / `refund_id`**——**部分退款必须同时携带并分别校验原支付总额与本笔退款额（原额 ≠ 本笔额是常态，不得以退款额冒充原额）**；`original_total_fen` 与 `payment_snapshot_json` 微信腿 / `refund_aggregate` 原额一致，`refund_fen` 与 `refund_aggregate` 本笔分摊一致，任一不一致拒绝入账进入对账。退款通知状态 CAS 覆盖 **`dispatching | dispatch_unknown | accepted → confirmed_refunded`**——通知先到（分派结果未知时退款成功通知到达仍可推进至确认态）、分派响应丢失（查询与通知双通道均可推进，`query_generation` / `version + lease_token` CAS 防陈旧覆盖）。支付通知明确 **`prepay_ready → settling → succeeded`** 合法转换（`prepay_ready` 仅表示可调起会话，结算由支付成功事实驱动）。故障测试：**通知先到 / 分派响应丢失 / 部分退款（原额与本笔额分别校验）**。
...
| 命令 | 前置 | 动作 / 结果 | 约束 |
|---|---|---|---|
| 新建支付尝试 | 同主体无活跃尝试 | 创建 `draft` | **旧尝试确认终态（`failed / expired / cancelled`）前不得新建**；`manual_review` 未解除前禁止 |
...
| 结算 | `settling` | → `succeeded / failed` | 仅 `succeeded` 对同一交易号幂等 ACK |
| 释放预占 | 终态 | 释放 `account_hold` | 仅 `failed / expired / cancelled`；`settling` 与微信等待期禁止 |
| 创建退款聚合 | **对应支付尝试 `succeeded`** | 初始化 `refund_aggregate` + 额度行（同一 UoW） | `prepay_*` / `settling` / `manual_review` / `failed / expired / cancelled` 一律不得创建 |
...
- **（B3.4）退款通知状态机测试**：`ProviderRefundEvent` 固定 `original_total_fen / refund_fen / currency=CNY / out_refund_no`（部分退款原额 ≠ 本笔额分别校验，不得以退款额冒充原额）；退款通知 CAS 覆盖 `dispatching | dispatch_unknown | accepted → confirmed_refunded`（通知先到 / 分派响应丢失均可推进，`version + lease_token` 陈旧拒绝）；支付 / 退款查询事件键分命名空间（`pay_query_result` / `refund_query_result`）；故障用例：通知先到 / 响应丢失 / 部分退款。
