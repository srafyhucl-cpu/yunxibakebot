...
|---|---|---|---|---|---|---|
| 支付通知 `pay_notify` | 验签上下文 / `mchid` / `appid` / `trade_state=SUCCESS` / `amount.total` / `currency=CNY` / `out_trade_no` / `transaction_id` | `wechat:notify:<transaction_id>:<merchant_order_no>` | 金额 / 币种 / 商户单号与不可变快照微信腿一致 | `settling → succeeded`（仅 `succeeded` 幂等 ACK） | 持久化成功且事件 `processed` 才 200，否则 5xx | 逐字段缺失 / 不匹配负向用例 |
| 退款通知 `refund_notify` | 验签上下文 / `mchid` / `appid` / 退款成功状态 / `amount.total` / `currency=CNY` / `out_refund_no` / `refund_id` | `wechat:refund_notify:<refund_id>:<out_refund_no>` | 退款金额与 `refund_aggregate` 分摊一致 | `accepted → confirmed_refunded`（`version + lease_token` CAS） | 同上 | 退款号缺失 / 金额不一致负向用例 |
| 订单查询 `query_result`（支付） | `trade_state` / `amount.total` / `currency=CNY` / `out_trade_no` | `wechat:query_result:<merchant_order_no>:<query_generation>` | 快照微信腿一致 | 按固定映射 CAS（`settling` / 关单 / 终态） | 查询回写以 `query_generation` 条件更新，陈旧拒绝 | 陈旧代次回写拒绝用例 |
| 退款查询 `query_result`（退款） | `status` / `amount` / `out_refund_no` / `refund_id` | `wechat:query_result:<out_refund_no>:<query_generation>` | `refund_aggregate` 分摊一致 | `dispatch_unknown / accepted → confirmed_refunded / confirmed_not_refunded`（`version + lease_token + query_generation` CAS） | 同上 | 陈旧代次回写拒绝用例 |
...
- **逐端点迁移表（D1 实施验收，旧写路径禁用或转发）**：
  ...
  | `payment_notification.handle_wechat_payment_notify` | 回调处理 | 先持久化 `payment_provider_event` 再处理（租约 CAS，仅 `processed` 后 200） | 验签 + 固定校验清单 | 先持久化再 ACK | 转发统一入口 | 先持久化再 ACK 测试 |
  | `stored_value/payment.py` 两条路径 | 余额 / 组合支付 | 统一入口 + `payment_attempt` 主体分派 | 对应余额可结算 | 同 UoW 结算 | 转发统一入口 | 同 UoW 结算测试 |
  | `recharges.py` mock-pay / cancel | 充值写路径 | 充值尝试入 `payment_attempt`（`subject_type=recharge`，`provider=wechat` 时 `merchant_order_no` 唯一） | 状态机前置 | 创建 / 关单同一 UoW | 转发统一入口 | 回调归属测试 |
  ...
  | 充值写路径 `recharges.py create / cancel / expire` | 充值单创建 / 取消 / 过期 | `payment_attempt`（`subject_type=recharge`）+ 统一命令 | 状态机前置 | 创建 / 关单 / 过期同一 UoW | 转发统一入口 | 充值回调归属测试 |
- **静态守卫（B3.3，评审问题 2）**：D1 实施新增「**绕过统一支付应用服务即失败**」静态检查（并入 `scripts/check_project.py` 或独立 check 脚本，进 pre-commit 门禁）——扫描全部结算 / 快照 / 退款 / 关单写入口，凡直接写 `order.payment`、`payment_attempt`、`account_hold`、`ledger_operation`、`accounting_outbox` 而未先经统一支付应用服务分派的路径一律检查失败；上表每行对应一条守卫规则与回归测试，**不再保留迁移兜底行（逐行封闭核对）**。
...
- **（B3）券观察测试**：外部 `CONSUME / BACK / TAKE` 只写 `coupon_observation` 不改变当前态；无法精确匹配时**追加对账案件（`coupon_reconcile_case`）并由本地裁决命令置 `reconcile_hold`**（B3.1 + B3.2：观察只建案件、本地命令才改当前态，不可用、不可核销）；可精确匹配（同 `origin_event_id` 且版本可比）才更新投影。
...
- **（B3.3）积分两命令测试**：未结算（无 `points:redeem` 流水）订单经四类取消入口（用户取消 / 后台取消 / 单笔超时 / 批量超时）只清快照不 credit（余额断言不变）、`points_refund_reconcile` 只追加一条 `redeem_missing`（同订单同原因幂等）；已结算退款正常退回并收回、不产生对账记录；`award` 流水缺失时跳过收回并记 `award_missing`。
- **（B3.3）迁移与分类矩阵测试**：D1-0 封闭迁移表逐行核对（旧写路径禁用 / 转发，每行有回归测试）；「绕过统一支付应用服务即失败」静态守卫命中即失败；历史单据按分类矩阵处置（可重建回填 `snapshot_hash` 校验 / `legacy_payment_case` 冻结 / 已支付可退款回填额度行 / 退款资格有可追溯归属，禁止裸 `manual_review`）。
