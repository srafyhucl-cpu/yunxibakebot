...
- summary: P18b 新增 LangSmith 生产启用操作包，固定环境变量、启用前 strict gate、启用后观测命令、人工合规确认和回滚命令。默认采样率 `0.05`，不超过 P18a 安全上限；本切片不代表生产外发已打开，P18c 仍需生产仓库外注入 key/project/tracing 后复验。
