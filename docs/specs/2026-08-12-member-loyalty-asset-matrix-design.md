...
- **唯一活跃尝试（B1.9 + B3.1 补 `manual_review`）**：每个 `(subject_type, subject_id)` 至多一个活跃尝试——新尝试以条件更新创建（`WHERE 不存在 status ∈ {draft, prepay_requested, prepay_unknown, prepay_ready, settling, manual_review} 的同主体尝试`——**`manual_review` 未解除前禁止同主体创建新支付尝试，B3.1**），冲突即拒绝；**重复通知仅对 `succeeded` 同交易号幂等 ACK**（不重复结算）；**冲突或过期尝试进入对账**，不静默覆盖。
