...
- **支付尝试状态机（B2.0 完整定义 + B3 补调度 + B3.1 补状态）**：`draft → prepay_requested → prepay_unknown / prepay_ready → settling → succeeded / failed / expired / cancelled`（含 **`manual_review`** 处置态）——创建为 `draft`；发起微信预下单写 `prepay_requested`；预下单响应未知 `prepay_unknown`（**B3 + B3.1 固定映射：由独立调度任务按退避经 outbox 发起 `wechat_order_query` 并 CAS 推进，与关单互斥、lease_token 防陈旧回写——查询 `SUCCESS` → `settling`；`NOTPAY` → 先关单确认再 `cancelled / expired` 终态释放；`CLOSED / REVOKED / PAYERROR` → 终态释放；未知 → 保持占用退避查询至上限转 `manual_review`；`prepay_unknown` 不产生 `prepay_ready`（prepay_id 丢失不可恢复），支付会话禁止写回不可变快照；**用户侧重试由调度先推进原尝试至确认终态（查询-关单路径）或 `manual_review`（保持占用），终态确认后才允许新建尝试（B3.2）**）、就绪 `prepay_ready`（**仅已持久化可用会话**）；回调 / 余额扣款开始结算置 `settling`；**仅 `succeeded` 对同一交易号回调幂等 ACK；`settling` 必须可恢复（进程重启后按快照继续）且不能提前确认成功**；`failed / expired / cancelled` 为终态永久失效；**`manual_review`（B3.1）为人工处置态——未解除前禁止同主体创建新支付尝试，holds 保持占用**。
