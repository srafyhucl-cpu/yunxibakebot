...
10. **第一次真实支付前硬门禁**：按 **ADR 0008 D1-C 既定政策**（全额 + 部分净额退款、差分分摊、逐腿额度）落地 `refund_aggregate + refund_operation`（原支付快照、各资产分摊、每步幂等键、重试与人工复核、微信已退本地未补偿的恢复策略）；项目负责人**仅裁决参数**（退款时限、审批阈值、金额上限等，默认时限 72h）；评审通过后方可进入阶段二受控测试。
