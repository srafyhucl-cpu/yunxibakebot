...
- **上游版本合同（B1.8）**：`inbox_events.id` 只作回放游标，**不充当上游因果顺序**。每个聚合保存**上游版本 / 原始事件 ID**（积分 `unique_id`、券 `origin_event_id`、身份 / 会员卡事件主键）；重建按聚合的上游版本比较胜出：
  - 事件的上游版本严格更新 → 覆盖投影；
  - 上游版本相同或可证实为同一事件（幂等）→ 跳过；
  - **无可靠上游版本的冲突事件 → 进入对账队列**（`import_reconcile_queue`），不静默覆盖，批次收尾统一上报。
...
- **按资产的快照版本与重建规则（B1.7 + B1.8）**：
  ...
  | 资产 | 快照版本来源 / 上游版本 | 重建规则 |
  |---|---|---|
  | 积分余额 `member_balance.points` | `points_snapshot_version`（导入时固化）/ 事件 `unique_id` | 按上游版本比较：事件更新则覆盖投影；无法比较进对账队列；积分流水由 Webhook 增量维护，重建不写流水 |
  | 优惠券 `coupon_events` / `coupon_current_state` | `coupon_snapshot_version` / `origin_event_id` + `transition_key` | 迁移行 `legacy:<coupon_inventory.id>` 后，按 `origin_event_id` / `transition_key` 追加窗口内事件并重算投影 |
  | 会员身份 `customer_identity_links` | `identity_snapshot_version` / 事件主键 | 上游版本更新才覆盖归属，冲突进对账队列 |
  | 会员卡 `member_balance` 卡字段 | `card_snapshot_version` / 事件主键 | 上游版本更新才覆盖卡状态，冲突进对账队列 |
...
- **快照冲突胜出规则**：以每资产上游版本（非到达顺序、非仅 `occurred_at`）比较——事件上游版本更新则覆盖投影 / 追加事件，同版本或更旧则跳过（旧事件不覆盖新快照）；无法比较进入对账队列；重建器不做入站幂等短路，覆盖判定完全由上游版本合同驱动。
- **乱序处理**：重建按 `id ASC` 单调推进（游标维度）；聚合内乱序 / 迟到以**上游版本合同**裁决，同时间戳事件以上游版本 + `origin_event_id` 消歧。
- **故障恢复测试矩阵（实施阶段必测，B1.7 补"旧快照覆盖后重建恢复"，B1.8 补"物化幂等 / 对账队列"）**：
  ...
  | 迟到 / 同时间 / 乱序事件 | 按上游版本合同胜出；无上游版本冲突进对账队列，不静默覆盖 |
