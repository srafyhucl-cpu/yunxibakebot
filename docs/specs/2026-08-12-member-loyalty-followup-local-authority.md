...
8. **切换 / roll-forward 合同（固定，不再三选一）**：
   - `transition_key` 精确定义（B1.8 + B2.0 + B3.3）：`sha256(coupon_id + ":" + mobile + ":" + transition_type + ":" + business_ref + ":" + cycle_no + ":" + payment_attempt_id)`（不含来源；`transition_type ∈ {TAKE, RESERVE, RELEASE, CONSUME, BACK, EXPIRE, RECONCILE_HOLD, RECONCILE_VERDICT}`（**B3.3 补 RECONCILE_* 本地对账裁决命令事件**），`payment_attempt_id` 仅订单路径携带，`RESERVE` 分配周期、`CONSUME` 复用）；`business_ref` 为 `order:<order_no>` / `refund:<refund_no>` / `take:<外部事件 ID>` / `import:<导入批次 ID>` / `legacy:<coupon_inventory.id>` / **`reconcile:<case_id>:<verdict_version>`（B3.3：`RECONCILE_*` 裁决命令携带案件引用与裁决版本）**（**legacy 同样走统一公式**，无独立例外）；`origin_event_id` 须为可跨 import/webhook 对齐的上游稳定 ID，导入批次行 ID 不默认等同 webhook `msg_id`，缺失隔离待对账。
